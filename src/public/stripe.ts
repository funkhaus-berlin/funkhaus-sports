...
export function appearance(): Appearance | undefined {
	return {
		...
		variables: {
			...
			fontSizeSm: '12px',
			colorPrimary: getComputedStyle(window.document.body).getPropertyValue(
				SchmancyTheme.sys.color.primary.default.slice(4, -1),
			),
			colorBackground: getComputedStyle(window.document.body).getPropertyValue(
				SchmancyTheme.sys.color.surface.highest.slice(4, -1),
			),
			borderRadius: '8px',
			blockLogoColor: getComputedStyle(window.document.body).getPropertyValue(
				SchmancyTheme.sys.color.primary.default.slice(4, -1),
			),
			colorText: getComputedStyle(window.document.body).getPropertyValue(
				SchmancyTheme.sys.color.primary.default.slice(4, -1),
			),
		},
		labels: 'above',
