...
//  * Uses JWT and native https for API calls
//  */
...
//   /**
//    * Helper method to make HTTP requests using Node.js native https
//    */
//   private async httpRequest(url: string, method: string, headers: any, data?: any): Promise<any> {
...
//             const error: any = new Error(`HTTP error ${res.statusCode}`);
//             error.response = { 
...
//       // Exchange JWT for access token using native https
...
//       this.tokenExpiry = Math.floor(Date.now() / 1000) + response.expires_in;
...
//         const getResponse = await this.httpRequest(
//           url,
//           'GET',
//           {
...
//         const updateResponse = await this.httpRequest(
//           url,
//           'PUT',
//           {
//             'Authorization': `Bearer ${token}`,
