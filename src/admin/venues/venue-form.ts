...
export class VenueForm extends $LitElement() {
        ...
        ${days.map(day => {
          ...
          return html`
            <div class="flex items-center gap-2 mb-2">
              <schmancy-checkbox 
                .checked=${isOpen}
                @change=${(e: CustomEvent) => this.toggleDayOperation(day.key as keyof OperatingHours, e.detail.checked)}
              ></schmancy-checkbox>
              <schmancy-typography class="w-28">${day.label}</schmancy-typography>
  ...
  updateAddress(prop: keyof Address, val: string | number) {
    const address = { ...(this.venue.address || {}), [prop]: val }
    this.venue = { ...this.venue, address }
  }
  ...
  toggleDayOperation(day: keyof OperatingHours, isOpen: boolean) {
    ...
    this.venue = { ...this.venue, operatingHours }
  }
  ...
  onSave = (e: Event) => {
    ...
    if (!this.venue.address?.street || !this.venue.address?.city || 
        !this.venue.address?.postalCode || !this.venue.address?.country) {
    ...
    // Save to database
    const saveOperation = this.editingVenue ? 
      VenuesDB.upsert(venue, this.editingVenue.id) : 
      VenuesDB.upsert(venue)
